...
public class RedPacket {
    ...
    private int randomRedPacket(final int totalMoney, int minMoney, int maxMoney, int count) {
        //只有一个红包直接返回
        if (count == 1) {
            return totalMoney;
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("运气王--》"+redPackets.stream().sorted().max((Integer a,Integer b)->a-b).get());
        System.out.println("大于平均值的个数-》"+redPackets.stream().filter((Integer a)-> a > 20000 / 100).count());
        int sum = 0;
